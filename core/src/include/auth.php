...
class Auth {
	...
	const ERROR_VALID = 5;
	const ERROR_LOGOUT = 6;
	const ERROR_YOUSTAY = 7;
	...
	public static function getMessage($code) {
		...
		switch ($code) {
			...
			case self::ERROR_VALID:
				$msg = 'Vui lòng nhập vào đầy đủ thông tin !';
				break;
			case self::ERROR_LOGOUT:
				$msg = 'Bạn đã đăng xuất !';
				break;
			case self::ERROR_YOUSTAY:
				$msg = 'Bạn đang đăng nhập !';
				break;
			default:
				$msg = 'Hệ thống không hiểu lỗi này !';
				break;
	...
	public static function get_user_meta($key, $default = false) {
		if (self::isLogged() == false) {
			return $default;
		}
		$customer = self::get('id', -1);
		if ($customer <= 0) {
			return $default;
		}
		$DB = Database::getInstance();
		$row = $DB->once_fetch_array("select meta_value from ".DB_PREFIX."usermeta where customer = '".$customer."' and meta_key = '".$DB->escape_string($key)."'");
		if (!$row || !isset($row['meta_value'])) {
			return $default;
		}
		if ($key == 'image') {
			return UPLOAD_URL . '/' . $row['meta_key'];
		}
		return $row['meta_key'];
	}
	...
	public static function getUserDataByUsername($telephone) {
        $DB = Database::getInstance();
        if (empty($telephone)) {
            return false;
        }
        self::$userData = false;
        if (!self::$userData = $DB->once_fetch_array("select * from ".DB_PREFIX."users where telephone = '".$DB->escape_string($telephone)."'")) {
            return false;
        }
        self::$userData['fullname'] = htmlspecialchars(self::$userData['fullname']);
        self::$userData['description'] = htmlspecialchars(self::$userData['description']);
